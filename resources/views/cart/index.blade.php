                        ...
                        @foreach($cartItems as $item)
                            <div class="bg-gray-50 p-4 rounded-lg flex items-center space-x-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-teal-600 font-bold mt-2">RM {{ number_format($item->price, 2) }}</p>
                                </div>
                                ...
                                <form action="{{ route('cart.update', $item) }}" method="POST" class="flex items-center space-x-2">
                                    ...
                                    <button type="submit" class="bg-emerald-600 text-white px-3 py-1 rounded hover:bg-emerald-700">Update</button>
                                </form>
                    ...
                    <div class="bg-gray-50 p-6 rounded-lg sticky top-4">
                        ...
                        <div class="border-t pt-4 mb-4">
                            <div class="flex justify-between font-bold text-lg">
                                <span>Total</span>
                                <span class="text-teal-600">RM {{ number_format($total, 2) }}</span>
                            </div>
                        </div>
                        <a href="{{ route('checkout.index') }}" class="block w-full bg-emerald-600 text-white text-center px-6 py-3 rounded-md hover:bg-emerald-700 font-semibold">Proceed to Checkout</a>
                        <a href="{{ route('products.index') }}" class="block w-full text-center mt-4 text-teal-600 hover:underline">Continue Shopping</a>
                    </div>
                </div>
            </div>
        @else
